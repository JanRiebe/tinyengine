...
int main(int argc, char ** argv)
{
    ...
    GLuint genShadowMapProgram = createShaderProgram("../Shaders/genShadowMap.vs",
        ...
        GLuint simpleShaderProgram = createShaderProgram("../Shaders/simple.vs", NULL, NULL,
	...
	Camera camera(Vec3(0.0f, -3.0f, 3.0f), Vec3(0.0f, 1.0f, -1.0f),
        ...
        Mesh suzanneMesh("../Meshes/suzane.obj", simpleShaderProgram);
	...
	Renderable renderable(suzanneMesh, simpleShaderProgram);
	...
	while (!finished)
	{
		...
		GL_CHECK_ERROR();
		//TODO extend renderable, so that it can do shadows
		...
		glUniformMatrix4fv(
                ...
                // Render a second object
				
				renderable.Draw(camera, light);



                // End of render the framebuffer
                glBindSampler(0, 0);
                GL_CHECK_ERROR();
                glBindTexture(GL_TEXTURE_2D, 0);
                GL_CHECK_ERROR();
