	...
	if (in)
	{
		...
		*image = new float[*ResX * *ResY * 3];
		...
		for (int j = 0; j < *ResY; j++)
		{
			for (int i = 0; i < *ResX * 3; i++)
			{
				float tmp = (float) buffer_c[i + j * *ResX * 3] / 255.0f;
...
bool load_image_ppm(const char *file, unsigned char **image, int *ResX, int *ResY)
{
	FILE *in = fopen(file, "rb");
	if (in)
	{
		if (!fscanf(in, "P6\n%d\n%d\n255\n", ResX, ResY))
			return false;
		*image = new unsigned char[*ResX * *ResY * 3];
		if (fread(*image, *ResX * *ResY * 3, 1, in) != 1)
			return false;
